...
//TODO: Create function to write one file inside a pbo object and output to a directory (modify pbo object, export out)
namespace PBOSharp
{
	public class PBOWriter : BinaryWriter
	{
		...
		internal void WritePBO(PBO pboName)
		{
			try
			{
				_client.PushOnEvent("Starting WritePBO", EventType.Debug);
				PBOPrefix prefix = pboName.Prefix;
				List<PBOFile> files = pboName.Files;
				...
				foreach (PBOFile file in files)
				{
					...
					byte[] readStuff = br.ReadBytes(file.DataSize);
					Write(readStuff);
					_client.PushOnEvent($"Wrote Data for: {file.FileName}", EventType.Info);
				}
